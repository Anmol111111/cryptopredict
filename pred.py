...
optimizer = 'adam'
final_data = {}
# for currency in crypt:
#     final_data[currency] = dict()
# print(final_data)
for i in crypt:
    ...
    pd = pd.Series(index=pd.date_range(start=date.today() + datetime.timedelta(days=1), end=date.today() + datetime.timedelta(days=10)), data=(max * pred[-1])+min)
